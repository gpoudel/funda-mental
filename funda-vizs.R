...
ggplot(df %>% filter(City == "Utrecht"), aes(substr(Postcode,1,4), Price,  colour = PropertyType)) + geom_boxplot() + facet_grid(City~.)+ theme_bw() + theme(axis.text.x = element_text(angle = 45, hjust = 1))
